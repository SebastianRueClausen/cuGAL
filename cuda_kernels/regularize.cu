...
__global__ void kernel(Accessor<float, 2> gradient, Accessor<float, 2> P, int iteration, size_t entry_count)
{
...
void regularize(torch::Tensor gradient, torch::Tensor P, int iteration)
{
    ...
    kernel<<<block_count, block_size>>>(
        gradient.packed_accessor32<float, 2>(),
        P.packed_accessor32<float, 2>(),
        iteration,
        entry_count);
