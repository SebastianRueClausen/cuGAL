...
struct Block {
    ...
    size_t output_size(size_t row_count) {
        return sizeof(float) * row_count * output.size(1);
    }

    void upload(float *data, size_t row_count) {
        check_cuda_error(cudaMemcpyAsync(
            input.data(), data, input_size(row_count), cudaMemcpyHostToDevice,
            stream
        ));
    }

    void download(float *data, size_t row_count) {
        check_cuda_error(cudaMemcpyAsync(
            data, output.data(), output_size(row_count), cudaMemcpyDeviceToHost,
            stream
        ));
    }
...
inline std::optional<Accessor<float, 2>> try_create_accessor(const long sizes[2]
    ...
    if (cudaMalloc(&memory, sizes[0] * sizes[1] * sizeof(float)) !=
        cudaSuccess) {
        return std::nullopt;
    }
    const long strides[2] = {sizes[1], 1};
    return std::optional<Accessor<float, 2>>{
        Accessor<float, 2>(memory, sizes, strides)
    };
}

inline std::optional<Block>
try_create_block(const long input_sizes[2], const long output_sizes[2]) {
