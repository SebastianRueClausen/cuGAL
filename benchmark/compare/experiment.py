...
from cugal.profile import Profile, TimeStamp, plot_times, plot_sinkhorn_iterations
...
class Graph(Enum):
    ...
    def get(self) -> tuple[nx.Graph, nx.Graph | None]:
        match self:
            case Graph.CA_HEP:
...
class Experiment:
    ...
    def to_dict(self) -> dict:
        ...
        dict['graphs'] = [graph.value for graph in self.graphs]
    ...
    def from_dict(cls, dict: dict):
        ...
        dict['graphs'] = [Graph[graph] for graph in dict['graphs']]
...
class ExperimentResults:
    ...
    def __init__(self, experiment: Experiment, results: list[list[list[Result]]]):
        self.experiment = experiment
        self.results = results
        self.time = str(datetime.datetime.now())
        self.commit = get_last_commit()
...
def test(experiment: Experiment, use_fugal=False) -> Result:
    ...
    if experiment.target is None:
        generated_graph = generate.generate_graphs(
            experiment.source,
        ...
        source_mapping = generated_graph.source_mapping
    else:
    ...
    accuracy = metrics.accuracy(source_mapping, mapping)
...
if __name__ == "__main__":
    ...
    experiment = Experiment(
        graphs=[Graph.CA_HEP],
        algorithms=[Algorithm(Config(), use_fugal=False)],
        noise_levels=[NoiseLevel(0.25, 0.0, False)],
    )
    results = ExperimentResults(experiment, [])

    print(json.dumps(results.to_dict(), indent=4))

    # graph = Graph.CA_HEP.get()
