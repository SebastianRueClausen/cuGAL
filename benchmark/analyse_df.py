...
def prompt_user_and_load_results():
    ...
    df = pd.DataFrame.from_dict(results.all_results())
    df.columns = ['graph', 'noise', 'algorithm', 'result']
    df['accuracy'] = [r.accuracy for r in df['result']]
...
def prompt_user_for_analysis_options(results: ExperimentResults) -> tuple[ExperimentResults, PlotType]:
    ...
    if average_graphs.lower() == 'y':
        ...
        combined_experiment.graphs = [Graph(GraphKind.AVERAGED, 
...
def bar_plot_results(results: ExperimentResults, df: DataFrame, plot: Plot):
    ...
    for i, res in df.iterrows():
        plt.bar(i, float(res['accuracy']), 
                label=res['algorithm_desc'],
                color=cmap(int.from_bytes(res['algorithm_desc'].encode(), 'little') % 20))

    plt.legend(df['algorithm_desc'].unique(), loc='center left', bbox_to_anchor=(1, 0.5))
    ...
    for ps0, ps1, lbl in zip(pos[:-1], pos[1:], df['graph_desc'].unique()):
    ...
    print("x_pos ", x_pos)
    plt.xticks(x_pos, [str(n.source_noise*100) + "% Noise" for n in df['noise'].unique()])
    ...
    #draw_x_axis_labels()
    plt.legend(df['algorithm_desc'], loc='center left', bbox_to_anchor=(1, 0.5))
...
def heatmap_plot_results(results: ExperimentResults, df: DataFrame, plot: Plot):
