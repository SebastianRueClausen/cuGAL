...
experiment = Experiment(
    ...
    algorithms=np.array([
        ...
        #Algorithm(replace(config, frank_wolfe_threshold=0.10), use_fugal=False),
        #Algorithm(replace(config, frank_wolfe_threshold=0.50), use_fugal=False),
        #Algorithm(replace(config, frank_wolfe_threshold=1), use_fugal=False),
        #Algorithm(replace(config, frank_wolfe_threshold=5), use_fugal=False),
        #Algorithm(replace(config, frank_wolfe_threshold=10), use_fugal=False),
        Algorithm(replace(config, frank_wolfe_threshold=1), use_fugal=False),
        Algorithm(replace(config, frank_wolfe_threshold=10), use_fugal=False),
        Algorithm(replace(config, frank_wolfe_threshold=100), use_fugal=False),
        Algorithm(replace(config, frank_wolfe_threshold=1000), use_fugal=False),
        Algorithm(replace(config, frank_wolfe_threshold=10000), use_fugal=False),
    ]).flatten(),
    noise_levels=[
